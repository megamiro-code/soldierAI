...
# Hierarchical encoder output = terrain + 200*32 + 2*32 = 6720
...
def nearest_enemy_features(local_x, local_z, state, perspective_team):
    local_team = teams
    alive = state["alive"]

    dx = local_x[None, :] - local_x[:, None]
    dz = local_z[None, :] - local_z[:, None]
    dist2 = dx * dx + dz * dz
    ...
    # perspective_team is used so the feature definition remains explicit;
    # all enemy filtering is already encoded by team inequality above.
    del perspective_team

    nearest_idx = jnp.argmin(jnp.where(valid, dist2, 1e9), axis=1)
...
def make_observation(state, perspective_team):
    ...
    nearest = nearest_enemy_features(local_x, local_z, state, perspective_team)
...
def init_policy(key):
    ...
    p = {
        "Ws": random.normal(k1, (SOLDIER_FEATURES, LOCAL_EMBED_SIZE)) * jnp.sqrt(2.0 / SOLDIER_FEATURES),
        "bs": jnp.zeros((LOCAL_EMBED_SIZE,)),
        "Wc": random.normal(k2, (COMMANDER_FEATURES, LOCAL_EMBED_SIZE)) * jnp.sqrt(2.0 / COMMANDER_FEATURES),
        "bc": jnp.zeros((LOCAL_EMBED_SIZE,)),
        "Wg1": random.normal(k3, (GLOBAL_INPUT_SIZE, HIDDEN1)) * jnp.sqrt(2.0 / GLOBAL_INPUT_SIZE),
        "bg1": jnp.zeros((HIDDEN1,)),
        "Wg2": random.normal(k4, (HIDDEN1, HIDDEN2)) * jnp.sqrt(2.0 / HIDDEN1),
        "bg2": jnp.zeros((HIDDEN2,)),
        "Wa": random.normal(k5, (HIDDEN2, ACTION_SIZE)) * 0.01,
        "ba": jnp.zeros((ACTION_SIZE,)),
        "Wv": random.normal(k6, (HIDDEN2, 1)) * 0.01,
        "bv": jnp.zeros((1,)),
    }
    del k7, k8
...
def policy_forward(params, obs):
    ...
    soldier_emb = jnp.tanh(soldier_part @ params["Ws"] + params["bs"])
    ...
    h1 = jnp.tanh(global_input @ params["Wg1"] + params["bg1"])
    h2 = jnp.tanh(h1 @ params["Wg2"] + params["bg2"])
...
def select_best_bout_index(match, key):
    """
    Select which of the evaluated games becomes this generation's Best Bout.

    Returns:
        (index, was_decisive)
        index        : which game (0 .. total_games-1) was selected
        was_decisive : True  -> the overall winner had at least one decisive
                                 (commander-kill) victory, and that game was chosen
                       False -> no decisive victory existed this generation,
                                 so a game was picked uniformly at random instead
    """
    winner = match["winner"]
    ...
    if len(idx) > 0:
        ...
        return int(idx[order[0]]), True

    # No decisive victory anywhere this generation: pick a random game so a
    # Best Bout is always produced. Every game in the evaluated set (both
    # sides, 64 games total for EVAL_GAMES_PER_SIDE=32) is eligible.
    total_games = match["result"].shape[0]
    fallback_idx = int(random.randint(key, (), 0, total_games))
    return fallback_idx, False


def determine_game_outcome(match, idx, candidate_params, elite_params):
    """
    Resolve the actual outcome of ONE evaluated game (identified by idx),
    independent of the generation-level match["winner"].

    This is the single source of truth for who "won" the selected Best Bout
    game, used identically whether the game was chosen decisively or by the
    random fallback in select_best_bout_index.
    """
    cand_is_red = bool(match["candidate_is_red"][idx])
    game_result = int(match["result"][idx])
    red_surv = int(match["red_surv"][idx])
    blue_surv = int(match["blue_surv"][idx])
    red_return = float(match["red_return"][idx])
    blue_return = float(match["blue_return"][idx])
    ...
    if game_result == 1:
        ...
        # Timeout (3) or both-commanders-killed draw (4): no decisive kill in
        # THIS game. Break the tie by survivor count for labeling purposes only.
        winner_team = 0 if red_surv >= blue_surv else 1

    if winner_team == 0:
        winner_params = params_red
        winner_label = "Candidate" if cand_is_red else "Elite"
    else:
        winner_params = params_blue
        winner_label = "Elite" if cand_is_red else "Candidate"

    return {
        "cand_is_red": cand_is_red,
...
def save_best_bout(
    ...
    data = {
        ...
        "winner_code": np.array(match["winner"], dtype=np.int32),  # generation-level (overall) winner
        "winner_label": np.array(winner_label),                     # THIS game's winner
        "winner_team": np.array(winner_team, dtype=np.int32),
        "result_code": np.array(int(bout["result"]), dtype=np.int32),
        "is_random_pick": np.array(1 if is_random_pick else 0, dtype=np.int32),
        ...
        "red_return": np.array(match["red_return"][best_idx], dtype=np.float32),
        "blue_return": np.array(match["blue_return"][best_idx], dtype=np.float32),
        "red_survivors": np.array(match["red_surv"][best_idx], dtype=np.int32),
        "blue_survivors": np.array(match["blue_surv"][best_idx], dtype=np.int32),
...
def policy_params_compatible(params):
    required_shapes = {
        "Ws": (SOLDIER_FEATURES, LOCAL_EMBED_SIZE),
        "bs": (LOCAL_EMBED_SIZE,),
...
def train(n_generations=N_GENERATIONS, resume=True):
    ...
    print(f"Soldier encoder    : {SOLDIER_FEATURES} -> {LOCAL_EMBED_SIZE}")
    print(f"Commander encoder  : {COMMANDER_FEATURES} -> {LOCAL_EMBED_SIZE}")
    print(f"Global network     : {GLOBAL_INPUT_SIZE} -> {HIDDEN1} -> {HIDDEN2}")
    ...
    for generation in range(start_generation, n_generations + 1):
        ...
        master_key, pick_key = random.split(master_key)
        best_idx, was_decisive = select_best_bout_index(match, pick_key)

        outcome = determine_game_outcome(
            match, best_idx, candidate_params, elite_params
        )
        ...
        bout = record_bout(
            outcome["params_red"], outcome["params_blue"], init_state
        )
        steps = int(bout["end_step"])
        ...
        red_alive_final = final_alive[RED_COMMANDER_INDEX] > 0
        blue_alive_final = final_alive[BLUE_COMMANDER_INDEX] > 0

        game_result = outcome["game_result"]
        if game_result == 1:
            commander_outcome_pass = red_alive_final and (not blue_alive_final)
        elif game_result == 2:
            commander_outcome_pass = blue_alive_final and (not red_alive_final)
        elif game_result == 4:
            commander_outcome_pass = (
                (not red_alive_final) and (not blue_alive_final)
            )
        else:  # timeout
            commander_outcome_pass = red_alive_final and blue_alive_final
        ...
        print(
            f"Selection           : "
            f"{'Decisive win' if was_decisive else 'RANDOM (no decisive win this generation)'}"
        )
        print(
            f"Winner              : {outcome['winner_label']} "
            f"({'Red' if outcome['winner_team'] == 0 else 'Blue'})"
        )
        print(f"Red return          : {outcome['red_return']:.6f}")
        print(f"Blue return         : {outcome['blue_return']:.6f}")
        print(f"Win time            : {steps * DT:.2f} s  ({steps} steps)")
        print(f"Red survivors       : {outcome['red_surv']}")
        print(f"Blue survivors      : {outcome['blue_surv']}")
        print(
            f"Game result code    : {game_result}  "
            f"({RESULT_TEXT.get(game_result, 'UNKNOWN')})"
        )
        ...
        path = save_best_bout(
            ...
            verification_pass,
            max_err,
            outcome["winner_params"],
            outcome["winner_label"],
            outcome["winner_team"],
            was_decisive,
        )
        best_bout_saved = True
        if replay_result != game_result:
            print(
...
def build_replay_html(bout_path, out_path=None):
    ...
    is_random_pick = (
        bool(int(d["is_random_pick"])) if "is_random_pick" in d else False
    )
...
try{
 ...
 function updateReplay(){const f=frameInfo(),i=f.i,a=f.a;timeline.value=String(i);const X0=DATA.x[i],Z0=DATA.z[i],A=DATA.alive[i],X1=i<DATA.steps?DATA.x[i+1]:X0,Z1=i<DATA.steps?DATA.z[i+1]:Z0,px=k=>X0[k]+(X1[k]-X0[k])*a,pz=k=>Z0[k]+(Z1[k]-Z0[k])*a;const redActions=DATA.redActions[i]||null,blueActions=DATA.blueActions[i]||null;let redAttackCount=0,blueAttackCount=0;
  ...
  const ra=Array.from(A.slice(2,102)).reduce((sum,v)=>sum+v,0),ba=Array.from(A.slice(102,202)).reduce((sum,v)=>sum+v,0);const verify=DATA.verificationPass?'<span class="pass">Replay verification: PASS</span>':'<span class="fail">Replay verification: FAIL</span><br>Max state error: '+DATA.maxStateError.toExponential(2);
  const pickLabel=DATA.isRandomPick?'<span style="color:#ffb84d">Randomly selected (no decisive win this generation)</span>':'<span style="color:#5fd97a">Decisive victory</span>';
  info.innerHTML='<b>Generation '+DATA.generation+'</b><br>'+pickLabel+'<br>Winner: <b>'+DATA.winnerLabel+'</b> ('+DATA.winnerSide+')<br>Battle time: '+DATA.winTime.toFixed(1)+' s<br>Replay time: '+f.t.toFixed(2)+' s<br>Step: '+i+' / '+DATA.steps+'<hr>Red soldiers: '+ra+'<br>Blue soldiers: '+ba+'<br>Red Commander HP: '+DATA.hp[i][0].toFixed(2)+'<br>Blue Commander HP: '+DATA.hp[i][1].toFixed(2)+'<hr><span class="attack">Red attacks: '+redAttackCount+'</span><br><span class="attack">Blue attacks: '+blueAttackCount+'</span><hr>Result: <b>'+DATA.resultText+'</b><br>'+verify;statusEl.textContent='Generation '+DATA.generation+' | Best Bout | '+f.t.toFixed(2)+' s';}
